...
export const WalletProvider: React.FC<Props> = ({ children }) => {
  ...
  useEffect(() => {
    const fetchExistingAccount = async () => {
      if (
        ...
        window.lukso &&
        Number(await (new Web3(window.lukso).eth as any).getChainId()) ===
  ...
  const connect = async () => {
    if (typeof window !== 'undefined' && window.lukso) {
      ...
      const chainId = Number(await (web3.eth as any).getChainId());
      ...
      try {
        ...
        accounts = await (web3.eth as any).requestAccounts();
        if (!accounts.length) {
        ...
        const signerAddress = (web3.eth.accounts as any).recover(
          siweMessage,
          signature as string
        );
