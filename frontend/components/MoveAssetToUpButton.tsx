...
import {LSP1GraveForwader, LSP7Mintable__factory, LSP9Vault__factory} from "@/contracts";
...
export default function MoveAssetToUpButton({asset, from} : {asset:string, from: string}) {
    ...
    const transferTokenToUP = async (tokenAddress: string) => {
        try {
            ...
            if(!await lsp1GraveForwaderContract.tokenAllowlist(upAddress, tokenAddress)) {
            ...
            let lsp7Tx = lsp7.interface.encodeFunctionData("transfer", [from, await signer.getAddress(), 1, 0, "0x"]);

            const lsp9 = LSP9Vault__factory.connect(from, provider);
            await lsp9.connect(signer).execute(0, asset, 0, lsp7Tx, {gasLimit: 400_00});
